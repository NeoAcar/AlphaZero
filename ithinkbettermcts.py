...
class Node:
    def __init__(self, args:'dict', state:'chess.Board', move_counter:'int', policy:'np.ndarray', depth:'int'=0, parent:'Node | None'=None, action:'int|None'=None, prior:'float|None'=None):
        self.args = args
        self.state = state
        self.parent = parent
        self.prior = prior
    ...
    def expand(self, model: 'ResNet') -> None:
        ...
        for i, state in enumerate(new_states):
            if state.is_checkmate():
                values[i] = -1000
            child = Node(self.args, new_states[i], self.move_counter+1, policies[i], self.depth+1, self, actions[i], probs[i])
...
class MCTS:
    ...
    def update_root(self, state: chess.Board) -> None:
        """
        Update the root of the tree to the node corresponding to the action taken.

        Parameters:
        -----------
        action : str
            The action (move) that was taken.
        """
        # Find the child node corresponding to the action
        for child in self.root.children:
            if child.state == state:
                self.root = child
                self.root.parent = None  # Detach from the old tree
                return
    ...
    def search(self, state: 'chess.Board', move_counter: 'int') -> np.ndarray:
        ...
        if self.root is None:
            ...
            self.root = Node(self.args, state, move_counter,policy)
        else:
            # Update the state in case of a mismatch
            self.update_root(state)
            min_depth = self.root.depth
        
        
        max_depth.clear()
        self.root.policy = gf.valid_policy((1 - self.args['dirichlet_epsilon']) * self.root.policy + self.args['dirichlet_epsilon'] * np.random.dirichlet(
            [self.args['dirichlet_alpha']] * self.args['action_space']), root_state)
